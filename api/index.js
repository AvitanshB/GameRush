...
mongoose
  .connect(
    "mongodb+srv://bhattavitansh:deadpool%40121@cluster0.ebloxmg.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
  )
  .then(() => {
...
app.post("/games/:gameId/request", async (req, res) => {
  try {
    ...
    console.log("🔥 Incoming join request:", { gameId, userId, comment });
    ...
    if (!Array.isArray(game.requests)) {
      game.requests = [];
    }
    ...
    if (alreadyRequested) {
      console.log("⚠️ Request already exists for user");
