...
const GameSetUpScreen = () => {
  ...
  const isAdmin = userId === route?.params?.item?.adminId?._id;
  ...
  console.log("requests", route?.params?.item?.requests);
  console.log("userId", userId);

  const sendJoinRequest = async (gameId) => {
    try {
      const response = await axios.post(
        `http://10.0.2.2:8000/games/${gameId}/request`,
        {
          userId,
          comment,
        }
  ...
  console.log("userId", userId);
  console.log("adminId", route?.params?.item?.adminId);
  ...
  const toggleMatchFullStatus = async (gameId) => {
    try {
      ...
      const response = await axios.post(
        "http://10.0.2.2:8000/toggle-match-full",
        { gameId }
      );
  ...
  {
    console.log("❗Bottom Bar Should Render:", isAdmin, userRequested);
  }
